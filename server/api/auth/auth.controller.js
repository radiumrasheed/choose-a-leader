...
/**
 * Send Default Password to Members
 *
 * @param req
 * @param res
 */
exports.requestPassword = function (req, res) {
    ...
    Member.findOne({ sc_number: { $regex: exp } }, function (err, member) {
        if (err) { return handleError(res, err); }
        if (!member) { return res.status(400).json({ message: "No record found for specified Enrollment Number." }); }
        ...
        if (member.phone.indexOf(req.body.phone) == -1) {
            ...
            User.findOne({ username: req.body.sc_number.toLowerCase() }, '+clear_password', function (err, user) {
                if (err) { return handleError(res, err); }
                if (user) {
                    // Send Clear Password As Email and Text Message
...
exports.update = function(req, res) {
    ...
    if (typeof member._branch != "string") { delete member._branch; }
...
exports.sendCode = function (req, res) {
    User.findById(req.user).populate('_member').exec(function (err, user) {
        ...
        if (user._member.accessCode==undefined || user._member.accessCode=='' || user._member.accessCode == null) { return res.status(400).json({message:"Please setup account first."}); }
...
exports.confirm = function (req, res) {
    User.findById(req.user).populate('_member').exec(function (err, user) {
        ...
        Member.findById(user._member._id, function (err, member) {
            ...
            if (oldConf) { return res.status(200).json({message:"Access Code Confirmed"}); }
...
exports.changePassword = function (req, res) {
    ...
    if (req.body._id === undefined) { return res.status(400).json({ message: 'Invalid password reset request.' }); }
...
exports.signIn = function(req, res) {
    User.findOne({ username: req.body.username.toLowerCase(), role: (req.body.admin?'admin':'member') }, '+password', function(err, user) {
        if (!user) {
